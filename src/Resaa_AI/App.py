...
# ══════════════════════════════════════════════════════════════════════════════
# Model A — Property Price Prediction (existing)
# ══════════════════════════════════════════════════════════════════════════════
...
# Global land average — used as ROI baseline
global_land_avg = df_ref[
    df_ref["property_type"] == "قطعة أرض"
]["price_per_sqm"].mean()

CAT_COLS = ["classification", "property_type", "neighborhood", "plan_no", "parcel_no"]

# Minimum transactions to show a recommendation
MIN_TX = 10

# Permitted uses per classification — based on real data + legal zoning
PERMITTED_USES = {
...
# ══════════════════════════════════════════════════════════════════════════════
# Model B — Auction Starting Price Strategy
# Reads metadata from auction_model_meta.json saved by train.py.
# This guarantees the exact same feature list, bucket labels, ratio maps,
# and category sets that were used during training — no recomputation needed.
# ══════════════════════════════════════════════════════════════════════════════
...
# ── Feature list and categorical columns (exact order from training) ──────────
_AUCTION_FEATURES        = _meta["features"]
_AUCTION_CAT_COLS        = _meta["cat_cols"]

# ── Bucket modes (most frequent training bucket label) ────────────────────────
...
# ── Smoothed ratio maps (computed from training split in train.py) ────────────
...
# ── Load reference data for start price sweep range ──────────────────────────
...
# Training split reference (for start-price sweep range only)
...
def build_features(data):
    """Build feature row for Model A (property price prediction)."""
...
def _safe_group(value, known_set):
    """Map an inference-time category value to 'Other' if unseen during training."""
    value = str(value).strip()
    return value if value in known_set else "Other"


def _apply_auction_history(data):
    """Attach smoothed ratio history features using maps saved from training."""
    data = data.copy()
...
def recommend():
    ...
    1. Filters by legal classification (PERMITTED_USES)
    2. Excludes current property type (no point recommending what it already is)
    3. Predicts value for each permitted type using CatBoost
    4. Ranks by value_multiplier vs global land average
    5. If user provides budget → calculates ROI
    """
    try:
        data                = request.get_json()
        area                = float(data["area_sqm"])
        ...
        # ROI inputs — optional, only if user provides budget
        ...
        for ptype in permitted:
            ...
            mask     = (
                (df_ref["neighborhood"]  == neighborhood) &
                (df_ref["property_type"] == ptype)
            )
            ...
            if has_roi:
                profit = total_val - total_investment
...
def auction_strategy():
    """
    Auction starting price recommendation:
    1. Takes property details + seller's target price
    2. Sweeps a grid of (start_price × time_period × day_group × duration)
    3. CatBoost predicts sale_ratio for each scenario
    4. Returns the top strategy most likely to reach the target price
    5. Optional: return_top_n for showing ranked alternatives

    Required fields: area, target_price
    Optional fields: city, district, region, property_type,
                     n_price_steps (default 12),
                     durations (default [24, 48, 72]),
                     return_top_n (default 5)
    """
    try:
        data          = request.get_json()
        ...
        # ── Group inputs to known categories ──────────────────────────────────
        ...
        # ── Determine start price sweep range using training reference data ───
        ...
        sp_min = max(
            1000,
            min(subset["start_price"].quantile(0.15), 0.15 * target_price)
        )
        sp_max = min(
            max(subset["start_price"].quantile(0.85), 0.75 * target_price),
            0.95 * target_price
        )
        if sp_max <= sp_min:
            sp_max = sp_min * 2
                        ...
                        for sg in season_groups:
                            for dur in durations:
                                ...
                                rows.append({
                                    ...
                                    # bucket features — exact mode values from training
        ...
        # Ensure all expected features are present with correct types
        ...
        if valid.empty:
            # Fall back — return closest scenario even if below target
        ...
        valid["return_score"] = (
            valid["expected_return"] / (valid["expected_return"].max() + 1e-6)
        )
        valid["strategy_score"] = (
            0.70 * valid["target_score"] +
            0.30 * valid["return_score"]
        )

        top = valid.sort_values(
            by=["strategy_score", "target_score", "expected_return"],
            ascending=[False, False, False]
        ).head(return_top_n).reset_index(drop=True)
...
def neighborhoods():
    nbhds = sorted(df_ref["neighborhood"].dropna().unique().tolist())
    return jsonify(nbhds)
